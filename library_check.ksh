...
Exception: $LIB - Wrong Library Name or Not Exists
...
ssh -t -q $LHOST << EOF > /tmp/ttest.txt 2>/dev/null
df -kh $LDIR | tail -1
echo
df -km $LDIR
EOF
cat /tmp/ttest.txt | grep -iv "Bad file descriptor" | grep -iv "Thus no job control in this shell" | grep % > /tmp/ttest2.txt

Size=`cat /tmp/ttest2.txt | head -1 | uniq | sort |awk '{print $2}'`
Used=`cat /tmp/ttest2.txt | head -1 | uniq | sort |awk '{print $3}'`
Avail=`cat /tmp/ttest2.txt | head -1 | uniq | sort |awk '{print $4}'`
Usage=`cat /tmp/ttest2.txt | head -1 | uniq | sort |awk '{print $5}'`
Partition=`cat /tmp/ttest2.txt | head -1 | uniq | sort |awk '{print $6}'`
MAvail=`cat /tmp/ttest2.txt | tail -1 | uniq | sort | awk '{printf("%.f\n", $4/1024,"GB")}'`
}

mkdir -p tmp ; test -f tmp/library_check.log || touch tmp/library_check.log

function create_partition() {
if [[ $MAvail -lt 12 ]];then
echo "Warning : Only $Avail is available out of $Size of $Partition partition, So couldn't create backup."
else
echo "Info: $Avail is available out of $Size of $Partition partition.";echo
echo -n "Do you want to create a backup file of files in input dir of Access $AccessID?(y/n) : "
read OPT2
if [[ ( "$OPT2" != "n" ) && ( "$OPT2" != "y" ) ]];then
echo
...
CMD1='
echo "Local Directory of Access of Library :"
echo "--------------------------------------------------------------"
'
CMD2='
echo "Total File Count of Local Dir : "
echo "-----------------------------------"
'

CMD3='
echo "Backup file is created in below dir on GD Host :"
echo "--------------------------------------------------------------"
'

CMD4='
echo "Partition Status of Local dir of Access :"
echo "---------------------------------------------------------------"
'

CMD4='
echo "Current Partition Status of Local dir of Access :"
echo "---------------------------------------------------------------"
'

ssh -q $LHOST << EOF > /tmp/test3.txt 2>/dev/null
cd $LDIR
$CMD1
pwd
echo
$CMD2
find . -maxdepth 1 -type f -mtime -7 -exec ls -lrth {} \; | egrep "${MASK}" | wc -l
echo
$CMD3
find . -maxdepth 1 -type f -mtime -7 | egrep "${MASK}" | xargs tar -czf ../${LibN}_BKP_`date +"%Y%m%d-%H%M"`.tar.gz
find ../ -maxdepth 1 -type f -mmin -10 -iname "*${LibN}_BKP_`date +"%Y%m%d-%H%M"`.tar.gz*" | xargs realpath
echo
$CMD4
df -kh .
EOF
cat /tmp/test3.txt | grep -iv "Bad file descriptor" | grep -iv "Thus no job control in this shell" | grep -iv "failed" | grep -iv "login attempts"> /tmp/test4.txt
echo "============================================================================================================
"
cat /tmp/test4.txt
fi
...
do
if [[ ( "$Status_2" == "$Status_3" ) && ( $LDIR != NA ) ]];then
echo "Warning: $ConductorName is in Warning State.

So checking partition status to create backup of local dir of Access $AccessID
"
check_partition
echo "============================================================================================================

Partition Status of $HostN :
----------------------------------------------------
